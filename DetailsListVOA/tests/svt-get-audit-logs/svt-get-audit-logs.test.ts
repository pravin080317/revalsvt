...
describe('resolveTaskIdForAuditLogs', () => {
  ...
  test('prefers selectedTaskId when provided and strips prefix', () => {
    expect(saleDetailsRuntimeSource).toContain('const selected = normalizeTextValue(selectedTaskId);');
    // Early return if selected is truthy — now strips prefix to digits
    expect(saleDetailsRuntimeSource).toMatch(/if \(selected\)\s*\{\s*return stripTaskIdPrefix\(selected\);/);
  });

  test('stripTaskIdPrefix helper extracts digits only', () => {
    expect(saleDetailsRuntimeSource).toContain("const stripTaskIdPrefix = (value: string): string =>");
    expect(saleDetailsRuntimeSource).toContain("value.replace(/\\D/g, '')");
  });

  test('functional: strips A- prefix from selectedTaskId', () => {
    const { resolveTaskIdForAuditLogs } = require('../../services/runtime/sale-details');
    const result = resolveTaskIdForAuditLogs('{}', 'A-1029606');
    expect(result).toBe('1029606');
  });

  test('functional: strips M- prefix from taskDetails.taskId', () => {
    const { resolveTaskIdForAuditLogs } = require('../../services/runtime/sale-details');
    const json = JSON.stringify({ salesVerificationTaskDetails: { taskId: 'M-1029609' } });
    const result = resolveTaskIdForAuditLogs(json);
    expect(result).toBe('1029609');
  });

  test('functional: returns digits-only taskId unchanged', () => {
    const { resolveTaskIdForAuditLogs } = require('../../services/runtime/sale-details');
    const result = resolveTaskIdForAuditLogs('{}', '1029606');
    expect(result).toBe('1029606');
  });

  test('functional: returns empty string when no taskId', () => {
    const { resolveTaskIdForAuditLogs } = require('../../services/runtime/sale-details');
    const result = resolveTaskIdForAuditLogs('{}');
    expect(result).toBe('');
  });
