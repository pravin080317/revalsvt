...
describe('ViewSaleRecordById response model', () => {
  ...
  describe('salesVerificationTaskDetails mapping', () => {
    ...
    test('assignedToName is a display name (not a GUID)', () => {
      expect(getValue(task, 'assignedToName')).toBe('Jane Smith');
      expect(getValue(task, 'assignedToName')).not.toMatch(/^[0-9a-f]{8}-/i);
    });

    test('null name fields resolve to empty string via getValue', () => {
      expect(getValue(task, 'requestedByName')).toBe('');
      expect(getValue(task, 'qcAssignedToName')).toBe('');
    });
  ...
  describe('mapping with populated fields (non-null scenario)', () => {
    ...
    test('name fields are populated display names (not GUIDs)', () => {
      expect(getValue(task, 'assignedToName')).toBe('Jane Smith');
      expect(getValue(task, 'requestedByName')).toBe('Bob Requester');
      expect(getValue(task, 'qcAssignedToName')).toBe('Alice QC Reviewer');
    });
    ...
    test('mapPadConfirmationToKey maps the three gateway-accepted values', () => {
