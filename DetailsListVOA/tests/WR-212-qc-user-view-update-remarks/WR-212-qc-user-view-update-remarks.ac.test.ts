...
describe('WR-212 QC user can view sale record and update QC remarks AC', () => {
  ...
  test('AC4: QC fail requires remarks and maps task status to Assigned QC Failed; pass maps to Complete Passed QC', () => {
    ...
    expect(sectionSource).toContain("return 'Submit QC outcome';");
  });
  ...
  test('AC8: QC submit button text is dynamic based on pass/fail outcome selection', () => {
    // resolveQcSubmitButtonText uses sentence case per GDS content standards
    expect(sectionSource).toContain("const resolveQcSubmitButtonText = (outcomeKey?: string): string =>");
    expect(sectionSource).toContain("return 'Submit QC pass';");
    expect(sectionSource).toContain("return 'Submit QC fail';");
    expect(sectionSource).toContain("return 'Submit QC outcome';");
    // Call site passes qcOutcomeKey
    expect(sectionSource).toContain("resolveQcSubmitButtonText(qcOutcomeKey)");
  });

  test('AC9: QC submit button uses government-standard green styling and is center-aligned', () => {
    const cssSource = readRepoFile('DetailsListVOA/components/SaleDetailsShell/SaleDetailsShell.css');
    // QC button uses DefaultButton with action-btn class (same green style as caseworker buttons)
    expect(sectionSource).toContain('className="voa-sales-verification-action-btn"');
    expect(sectionSource).toContain('text={qcSubmitButtonText}');
    expect(sectionSource).toContain('ariaLabel={qcSubmitButtonText}');
    // QC actions container is center-aligned
