 ...
 * then picks only the API-approved field names (mirrors the real controller).
 */
function buildSubmitPayload(detailsJson: string, currentUserId?: string): string {
...
describe('SvtSubmitSalesVerification model', () => {
  ...
  describe('PCF merge logic alignment', () => {
    ...
    test('mergeSalesParticularDraftDetails maps particularNotes correctly', () => {
      expect(saleDetailsSource).toContain('particularNotes: emptyToNull(draft.particularsNotes)');
    });

    test('salesVerificationDetails fields use emptyToNull', () => {
      expect(saleDetailsSource).toContain('isSaleUseful: emptyToNull(payload.isSaleUseful)');
      expect(saleDetailsSource).toContain('whyNotUseful: emptyToNull(payload.whyNotUseful)');
      expect(saleDetailsSource).toContain('additionalNotes: emptyToNull(payload.additionalNotes)');
      expect(saleDetailsSource).toContain('remarks: emptyToNull(');
    });

    test('stripTaskIdPrefix is exported and used in controller submit flow', () => {
      expect(saleDetailsSource).toContain('export const stripTaskIdPrefix');
      const controllerSource = readRepoFile('DetailsListVOA/services/DetailsListRuntimeController.ts');
      expect(controllerSource).toContain('stripTaskIdPrefix');
      expect(controllerSource).toContain("stripTaskIdPrefix(rawTask.taskId)");
    });
  });
  ...
  describe('merge + build end-to-end flow', () => {
    ...
    test('Complete action sets taskStatus to Complete', () => {
      ...
      const submitJson = buildSubmitPayload(merged, 'cc4cc1fc-aeb8-ef11-b8e8-002248c64505');
      ...
      expect(submit.salesVerificationTaskDetails!.requestedBy).toBe('cc4cc1fc-aeb8-ef11-b8e8-002248c64505');
    ...
    test('remarks default to null when not provided for Complete', () => {
      const initial = buildInitialState();
      const payload: SalesVerificationActionPayload = {
        isSaleUseful: 'Yes',
        whyNotUseful: '',
      ...
      expect(submit.salesVerificationDetails!.remarks).toBeNull();
    });
    ...
    test('promoted WLTT master sets wlttId and clears lrppdId', () => {
      ...
      // legacy aliases stripped by pick — not sent to gateway
      expect(task.wltId).toBeUndefined();
      expect(task.lrpddId).toBeUndefined();
    });
    ...
    test('padConfirmation text mapping for job-created key', () => {
      const initial = buildInitialState();
      const payload: SalesVerificationActionPayload = {
        isSaleUseful: 'Yes',
        whyNotUseful: '',
    ...
    test('empty draft fields become null in submit payload', () => {
      const initial = buildInitialState();
      const payload: SalesVerificationActionPayload = {
        isSaleUseful: 'Yes',
        whyNotUseful: '',
      ...
      const submitJson = buildSubmitPayload(merged);
      const submit = JSON.parse(submitJson) as SubmitPayloadBody;
      const sp = submit.salesParticularDetails!;

      // Populated fields stay as-is
      expect(sp.kitchenAge).toBe('Dated');
      expect(sp.salesParticular).toBe('Details available');
      expect(sp.padConfirmation).toBe('Data enhancement job created');
      ...
      expect(sp.linkParticulars).toBeNull();
      expect(sp.kitchenSpecification).toBeNull();
      expect(sp.bathroomAge).toBeNull();
      expect(sp.bathroomSpecification).toBeNull();
      expect(sp.glazing).toBeNull();
      expect(sp.heating).toBeNull();
      expect(sp.decorativeFinishes).toBeNull();
      expect(sp.conditionScore).toBeNull();
      expect(sp.conditionCategory).toBeNull();
      expect(sp.particularNotes).toBeNull();
    });

    test('requestedBy is set to current user ID in submit payload', () => {
      const initial = buildInitialState({ requestedBy: undefined });
      const payload: SalesVerificationActionPayload = {
        isSaleUseful: 'Yes',
        whyNotUseful: '',
        additionalNotes: '',
      ...
      const merged = mergeSalesVerificationDetails(initial, payload, 'completeSalesVerificationTask');
      const userId = 'aabbccdd-1122-3344-5566-778899aabbcc';
      const submitJson = buildSubmitPayload(merged, userId);
      const submit = JSON.parse(submitJson) as SubmitPayloadBody;

      expect(submit.salesVerificationTaskDetails!.requestedBy).toBe(userId);
    });

    test('taskId with A- prefix is stripped to digits only', () => {
      const root = getEmptySaleRecord();
      root.salesVerificationTaskDetails = { taskId: 'A-1000555', taskStatus: 'Assigned' };
      const initial = JSON.stringify(root);
      const payload: SalesVerificationActionPayload = {
        isSaleUseful: 'Yes',
        whyNotUseful: '',
        additionalNotes: '',
      ...
      expect(submit.salesVerificationTaskDetails!.taskId).toBe('1000555');
    });

    test('taskId with M- prefix is stripped to digits only', () => {
      const root = getEmptySaleRecord();
      root.salesVerificationTaskDetails = { taskId: 'M-2000999', taskStatus: 'Assigned' };
      const initial = JSON.stringify(root);
      const payload: SalesVerificationActionPayload = {
        isSaleUseful: 'Yes',
        whyNotUseful: '',
      ...
      expect(submit.salesVerificationTaskDetails!.taskId).toBe('2000999');
    });

    test('taskId without prefix is left unchanged', () => {
      const root = getEmptySaleRecord();
      root.salesVerificationTaskDetails = { taskId: '1000555', taskStatus: 'Assigned' };
      const initial = JSON.stringify(root);
      const payload: SalesVerificationActionPayload = {
        isSaleUseful: 'Yes',
        whyNotUseful: '',
  ...
  describe('PCF ↔ Plugin field alignment', () => {
    ...
    test('PCF writes both particularNotes and particularsNotes', () => {
      expect(saleDetailsSource).toContain("particularNotes: emptyToNull(draft.particularsNotes)");
      expect(saleDetailsSource).toContain("particularsNotes: emptyToNull(draft.particularsNotes)");
    });
  });

  /* ---------------------------------------------------------------- */
  /*  Auto-navigate back to grid on submit success                    */
  /* ---------------------------------------------------------------- */

  describe('auto-navigate back on submit success', () => {
    const controllerSource = readRepoFile('DetailsListVOA/services/DetailsListRuntimeController.ts');
    ...
    test('controller exposes submitSuccessMessage getter and clearSubmitSuccessMessage', () => {
      expect(controllerSource).toContain('public get submitSuccessMessage(): string | undefined');
      expect(controllerSource).toContain('public clearSubmitSuccessMessage(): void');
    ...
    test('index passes submitSuccessMessage and onDismissSubmitSuccess to ControlShell', () => {
      expect(indexSource).toContain('submitSuccessMessage: this.runtime.submitSuccessMessage');
      expect(indexSource).toContain('onDismissSubmitSuccess: () => this.runtime.clearSubmitSuccessMessage()');
    });

    test('ControlShell renders success MessageBar with auto-dismiss', () => {
